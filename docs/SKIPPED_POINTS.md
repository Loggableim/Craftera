...
| AP | Grund | Datum | Nötig für Bearbeitung |
|----|-------|-------|------------------------|
| AP-8.2 … AP-8.5 | Die Provider-APs der Phase 8 (OpenAI, Anthropic, OpenRouter, Ollama lokal) benötigen eine externe Abhängigkeit zur realen Verifikation: API-Key (OPENAI_API_KEY/ANTHROPIC_API_KEY/OPENROUTER_API_KEY) bzw. einen laufenden lokalen Ollama-Server. Keine dieser Keys gesetzt, kein lokaler Ollama-Server erreichbar (localhost:11434). AP-8.6 (OpenAI-kompatibel) ist über den Ollama-Cloud-Endpoint (https://ollama.com/v1) mit den OLLAMA_FREE_KEY_*-Variablen real verifizierbar und wird separat bearbeitet. | 2026-08-12 | API-Key in der Umgebung setzen (OPENAI_API_KEY/ANTHROPIC_API_KEY/OPENROUTER_API_KEY) bzw. lokalen Ollama-Server starten. Danach AP-8.2 bis AP-8.5 bearbeiten. |
